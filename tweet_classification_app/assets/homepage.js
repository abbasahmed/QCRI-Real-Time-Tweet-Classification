var collection_socket = io.sails.connect();
var codes_array = [];
var collections_array = [];
collection_socket.get('/Tweets/search' , function(res){
  for(i in res.collections){
    collections_array.push(prettycode(res.collections[i]));
    codes_array.push(res.collections[i]);
  }
  autocomplete(document.getElementById("myInput"), collections_array, codes_array);
});

function autocomplete(inp, arr, vals) {
  ...
  inp.addEventListener("input", function(e) {
      ...
      for (i = 0; i < arr.length; i++) {
        ...
        if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
          ...
          b.innerHTML += "<input type='hidden' value='" + vals[i] + "'>";
